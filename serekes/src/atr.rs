...
impl AtrMonitor {
    ...
    pub async fn new(asset: &str) -> Self {
        let mut monitor = Self {
            periods: 12,
            symbol: format!("{}USDT", asset.to_uppercase()),
    ...
    async fn bootstrap(&mut self) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        ...
        let url = format!("https://api.binance.com/api/v3/klines?symbol={}&interval=5m&limit=13", self.symbol);
